...
function mostrarTabla() {
    ...
    let texto = limpiarTexto(document.getElementById('buscador')?.value || '');
    let filtrados = listaEnvios;
    
    if (texto !== '') {
        filtrados = listaEnvios.filter(e => {
            let destinatarioNorm = limpiarTexto(e.destinatario);
            let direccionNorm = limpiarTexto(e.direccion);
...
function editarEnvio(id) {
    ...
    if (!envio) {
        mostrarMensaje('Envío no encontrado', 'error', 'Error');
        return;
    }
